...
Blocked runs are retried by the daily schedule; they are not terminal and do not require a manual `run_now`. Each block records a `block_reason` on the run:

- `missing_metrics`: detected before the OpenRouter call, so retrying is free. Budget is 45 attempts.
- `ai_failure` and `unexpected`: every retry costs an OpenRouter call. Budget is 5 attempts.

A blocked run retries at most once per 20 hours (under 24 so a drifting daily cron never skips a day). Retry budgets and support-notice budgets are deliberately separate: notices go out on attempts 1, 4, 7, 10, and 13 and then stop, while retries continue silently. When the attempt budget runs out, the run is marked `retry_abandoned`, one final give-up notice is sent, and the run goes permanently quiet. That caps a single stuck run at six support emails.

Three ways to stop a blocked run: fix the underlying cause and let the next scheduled retry succeed, set `paused` on the client to drop it from due selection, or let the attempt budget expire. A manual `run_now` always retries regardless of `retry_abandoned` and resets it on success. Abandonment is per run and per period, so the next reporting month always starts with a fresh budget.
...
Manual `run_now` is an explicit override and may regenerate/resend same-period AM reviews. With `--client-id`, it runs only that client. Without `--client-id`, it runs every non-paused client regardless of `next_report_date` or the Sheet `run_now` flag. AM change requests also regenerate and resend the AM review. Scheduled daily runs must not regenerate an already open same-period `am_review` run; this restriction is about open AM reviews only and does not apply to blocked runs, which the schedule does retry.

`Messages` rows of type `processed_reply` are dedupe markers, not history. They are pruned automatically once they pass `REPLY_RETENTION`, which keeps the tab from growing without bound. Every other message type is kept permanently.
...
- The `Runs` tab carries `block_reason` and `retry_abandoned` columns after the standard timestamp columns. Both are written by `replace_rows`, so an older workbook gains them automatically on the first flush. Rows written before these columns existed fall back to inferring `missing_metrics` from a `No metrics found` error text.
...
- `REPLY_RETENTION` in `reportops/workflow.py` is a single 90-day window used for two things at once: `_active_thread_ids()` stops polling a run's Gmail threads that many days after the run was created, and `GoogleSheetsStore.prune_processed_markers()` expires `processed_reply` rows at the same age. Pruning is only safe because the polling window matches it, since Gmail thread fetches return every message in a thread with no date filter. Never shorten the marker retention below the polling window, or an already-answered client email can be answered twice. Only `processed_reply` rows are pruned; real message records are the audit trail and are kept forever.
- The reply-polling window is measured from `Run.created_at`, not `updated_at`, so a thread that keeps receiving replies cannot outlive the markers protecting it.
- Sheet reads use `MAX_SHEET_ROWS` rather than a small fixed row cap, and `replace_rows` pads shrinking tabs with blank rows. Both matter because every tab is read whole and rewritten from `A1`: a short read would strand rows below the rewritten block, and a short write would leave a stale tail behind.
...
- Keep scheduled retry of blocked runs, and keep the retry budget separate from the support-notice budget. Do not make a blocked run silent from the first attempt, and do not let it notify on every attempt forever.
- Keep `missing_metrics` retries free of OpenRouter calls. The metrics lookup must stay ahead of report generation so a data-gap block never costs money to retry.
